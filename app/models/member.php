...
class Member extends AppModel {
	...
	function password_match($check){
		...
		$value = $value[0];
		$valid = true;
		if (isset($this->data['Member']['new_password'])){
			if(sha1($this->data['Member']['new_password']) != sha1($value)){
				$valid = false;
			}
		}else{
	...
	function signup($data){
		$this->set($data);
		$validates = $this->validates(array("fieldList"=>array("firstname", "lastname", "password", "confirm_password", "email", "confirm_email")));
		if ($validates){
