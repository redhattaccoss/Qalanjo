				...
				<div class="profile-picture-bg left">
					<?php 
						if (isset($sentGift["MemberProfile"]["picture_path"])||($sentGift["MemberProfile"]["picture_path"]!="")){
							echo $html->image("uploads/".$sentGift["Sender"]["id"]."/default/profile_thumb_".$sentGift["MemberProfile"]["picture_path"], array("url"=>"/members/profile/".$sentGift["Sender"]["id"]));
						}else{
							if ($sentGift["Sender"]["gender_id"]==1){
								echo $html->image($path."default.jpg", array("url"=>"/view_members/profile/".$sentGift["Sender"]["id"]));
							}else{
								echo $html->image($path."woman-default-picture.jpg", array("url"=>"/view_members/profile/".$sentGift["Sender"]["id"]));
							}
						}
					?>
					</div>
