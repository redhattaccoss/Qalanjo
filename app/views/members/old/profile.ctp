...
<div class="content-container">
	<div class="view-profile-page-header">
		<div class="header-left-side">
			...
			<span class="back-to-my-matches-link">&laquo;<?php echo $html->link("Back to my matches", "/matches")?></span>
		</div>
		<div class="header-right-side">
			<?php
				...
				?>
					...
					<?php echo $html->link("Subscribe now!", "/subscribe")?>					
					</span>
				<?php 	
				}
			?>
		...
		<div class="left-side">
			<div class="introduction-box">
				...
				<?php if ($view_member["Member"]["id"]!=$member["Member"]["id"]){?>
					...
					<?php echo $html->link('Send a Gift', array('controller'=>'gifts'), array('class'=>'button last')); ?>
				<?php }?>
					<br/>
					<p class="status">
					<?php 
						if ($view_member["Member"]["online"]==1){
							echo $html->link("$pronoun_ref is online", "#", array("class"=>"online"));
						}else{
							echo $html->link("$pronoun_ref is last active last ".$time->timeAgoInWords($view_member["Member"]["modified"]), "#", array("class"=>"last-active"));
						}
					?>
					</p>
				</div>
			...
			<?php 
				if ($view_member["Member"]["id"]!=$member["Member"]["id"]){?>
					...
					<div class="what-to-do-with-this-user">
						<?php echo $html->link("Archive this Match", "/matches/archive/".$view_member["Member"]["id"])?>
					</div>
			<?php 
				}
			?>
				...
				<div class="content">
					<?php if ($view_member["Member"]["id"]!=$member["Member"]["id"]){?>
						...
						<ul>
								...
								<li>
									<div>
										<?php echo $html->image($path."bubble.png")?>
										<a href="#" id="icebreaker_link">Send FREE icebreaker</a>
									</div>
								</li>
								<li>
									<div>
										<?php echo $html->link("View $pronoun_self pictures", "/photos/".$view_member["Member"]["id"])?>
										<a href="#" id="icebreaker_link">Send FREE icebreaker</a>
									</div>
								</li>
								<li>
									<div>
										<?php echo $html->image($path."exit.png")?>
										<?php echo $html->link("Archive this Match", "/matches/archive/".$view_member["Member"]["id"])?>
									</div>
								</li>
					...
					<?php }else{
						...
						<ul>
							<li>
								<div>
									<?php 
										echo $html->link("Upload your photos", "/photos", array("class"=>"upload-button"));
