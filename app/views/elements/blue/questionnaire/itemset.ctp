...
<?php 
	...
	echo $form->create("MemberAttributeWeight", array("url"=>"/attributes/questionnaire/".($page+1)));
?>
<?php 
	$i = 1;
	$j=0;
			...
			<?php
				...
				foreach($itemSet as $question){
					...
					<li class="item clear">
               		...
               		<?php 
						$out = "";
						$out.=$form->input("MemberAttributeWeight.$j.weight", array("type"=>"hidden", "id"=>"weight".$question["Attribute"]["id"],"value"=>1));
						$out.=$form->input("MemberAttributeWeight.$j.attribute_id", array("type"=>"hidden", "value"=>$question["Attribute"]["id"]));
						$out.=$form->input("MemberAttributeWeight.$j.member_id", array("type"=>"hidden", "value"=>$member_id));
						echo $out;
						$j++;
					?>
					<?php 
