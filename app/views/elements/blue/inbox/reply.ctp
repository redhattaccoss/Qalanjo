	...
	foreach($message_replies as $reply){
		...
		<li class="left clear">
			<div class="sender-picture left">
				<?php 
					echo $html->image("uploads/$writer_id/default/profile_thumb_".$reply["Sender"]["MemberProfile"]["picture_path"], array("class"=>"left profile-picture", "alt"=>$name))
				?>
			</div>
			...
			<div class="right time">
				<?php echo $time->timeAgoInWords($reply["PrivateMessageReply"]["created"])?>
			</div>
		</li>
<?php }?>