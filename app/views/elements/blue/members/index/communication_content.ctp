				...
				<div class="activity-picture left">
					<?php 
						if (isset($message["MemberProfile"]["picture_path"])||($message["MemberProfile"]["picture_path"]!="")){
							echo $html->image("uploads/".$message["Member"]["id"]."/default/profile_thumb_".$message["MemberProfile"]["picture_path"], array("url"=>"/members/profile/".$message["Member"]["id"]));
						}else if ($message["Member"]["gender_id"]==1){
							echo $html->image($path."s-men.png", array("url"=>"/members/profile/".$message["Member"]["id"]));
						}else{
							echo $html->image($path."s-women.png", array("url"=>"/members/profile/".$message["Member"]["id"]));	
						}
					?>		
				</div>
				<div class="activity-content left">
					<div class="name-location left">														
						<h4 class="left"><?php echo $html->link($message["Member"]["firstname"]." ".$message["Member"]["lastname"], "/profile/".$message["Member"]["id"])?></h4>
					...
					<div class="activity-content-button-date left">
						<?php echo $html->link("<span class='left'>Show Message</span>", "/private_messages/read/".$message["ReceiveMessage"]["id"], array("class"=>"activity-content-button right", "escape"=>false))?>
				...
				<div class="activity-picture left">
					<?php 
						if (isset($wink["MemberProfile"]["picture_path"])||($wink["MemberProfile"]["picture_path"]!="")){
							...
							echo $html->image($path."s-men.png", array("url"=>"/Winkers/profile/".$wink["Winker"]["id"]));
						}else{
							echo $html->image($path."s-women.png", array("url"=>"/Winkers/profile/".$wink["Winker"]["id"]));	
						}
					?>		
				</div>
				<div class="activity-content left">
