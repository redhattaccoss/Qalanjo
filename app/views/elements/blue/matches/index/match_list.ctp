		...
		foreach($matches as $match){
			...
			<li>
				<div class="match-details">
					<div class="matches-list-image-holder">
						<?php 
							...
							if (isset($match["MemberProfile"]["picture_path"])||($match["MemberProfile"]["picture_path"]!="")){
								...
								echo $html->image($path."match-default-photo.jpg", array("url"=>"/members/profile/".$match["Matched"]["id"]));
							}else{
								echo $html->image($path."content-default-woman.jpg", array("url"=>"/members/profile/".$match["Matched"]["id"]));	
							}
						?>
					</div>
					<div class="matches-list-details">
						<div class="personal-info">
							<?php 
								echo $html->link($match["Matched"]["firstname"], "/members/profile/".$match["Matched"]["id"]);
							?>
							<span class="age">, <?php echo $match["Matched"]["age"]?> </span>
							<span class="match-status">
							<?php 
								if ($agoTime->dateMe($match["Match"]["created"])=="Today"){
									echo "NEW";
				...
				<div class="communication-stage">
					<?php 
						echo $html->link($match["MatchStatus"]["value"], "#");
					?>
				</div>
				<div class="next-steps">
					<?php 
						echo $html->link("View Match Detail", "/members/profile/".$match["Matched"]["id"]);
					?>
				</div>
			</li>
			<?php 
