		...
		<div class="profile-picture-bg left">
			<?php 
				if (isset($view_member["MemberProfile"]["picture_path"])||($view_member["MemberProfile"]["picture_path"]!="")){
					echo $html->image("uploads/".$view_member["Member"]["id"]."/default/profile_thumb_".$view_member["MemberProfile"]["picture_path"], array("url"=>"/members/profile/".$view_member["Member"]["id"]));
				}else{
					if ($view_member["Member"]["gender_id"]==1){
						echo $html->image($path."default.jpg", array("url"=>"/view_members/profile/".$view_member["Member"]["id"]));
					}else{
						echo $html->image($path."woman-default-picture.jpg", array("url"=>"/view_members/profile/".$view_member["Member"]["id"]));
					}
				}
			?>
		</div>
...
<div class="right icebreaker-form clear">
	<?php
		echo $form->create("SentIceBreaker", array("id"=>"breaker_form"));
		echo $form->label("SentIceBreaker.ice_breaker_id", "Ice breaker", array("class"=>"left"));
		echo $form->input("SentIceBreaker.ice_breaker_id", array(
							"type"=>"select", "options"=>$breakers,
							 "div"=>false, "label"=>false, "class"=>"left"));
		echo $form->input("SentIceBreaker.to_id", array(
							"type"=>"hidden","value"=>$view_id));
		echo $form->input("SentIceBreaker.member_id", array(
